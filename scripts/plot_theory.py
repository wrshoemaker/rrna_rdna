...
ax.plot(g_range, intercept + 0.5*numpy.log(g_range), lw=2, ls='-', c='#FF8A75', label='sublinear')
ax.plot(g_range, intercept + 1*numpy.log(g_range), lw=2, ls='-', c='#FF6347', label='linear')
ax.plot(g_range, intercept + 1.5*numpy.log(g_range), lw=2, ls='-', c='#CC4A35', label='superlinear')



ax.legend(loc='upper left', fontsize=9, title='Gene dosage effect')
...
delta_y = max(noise_ratio) - min(noise_ratio)
y_max = max(noise_ratio)
y_min = min(noise_ratio)

ax.fill_between(lag_time_range, y_max, where=lag_time_range >= 0, facecolor= utils.dna_rna_color_dict['RNA'], alpha=0.5, zorder=1)
ax.fill_between(lag_time_range, y_max, where=lag_time_range <= 0, facecolor= utils.dna_rna_color_dict['DNA'], alpha=0.5, zorder=1)
