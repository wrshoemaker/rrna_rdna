...
param_dict = plot_sine_parameters.load_param_otu_dict(log10_status=log10_status, clr_status=clr_status, otu_to_remove=None)
param_no_otu1_dict = plot_sine_parameters.load_param_otu_dict(log10_status=log10_status, clr_status=clr_status, otu_to_remove='Otu000001')


otu_labels = numpy.asarray(param_dict['otu_labels'])
otu_labels_no_otu1 = numpy.asarray(param_no_otu1_dict['otu_labels'])
...
fig = plt.figure(figsize = (4*len(data_type_all), 16))
fig.subplots_adjust(bottom= 0.15)


for data_type_idx, data_type in enumerate(data_type_all):

    amp_all = numpy.asarray(param_dict['amp_leastsq'][data_type])[to_plot_all_idx]
    amp_no_otu1 = numpy.asarray(param_no_otu1_dict['amp_leastsq'][data_type])[to_plot_no_otu1_idx]
    amp_merged = numpy.concatenate([amp_all, amp_no_otu1])
    amp_min_max = [min(amp_merged), max(amp_merged)]

    freq_all = numpy.asarray(param_dict['freq_leastsq'][data_type])[to_plot_all_idx]
    freq_no_otu1 = numpy.asarray(param_no_otu1_dict['freq_leastsq'][data_type])[to_plot_no_otu1_idx]
    freq_merged = numpy.concatenate([freq_all, freq_no_otu1])
    freq_min_max = [min(freq_merged), max(freq_merged)]

    phase_all = numpy.asarray(param_dict['phase_leastsq'][data_type])[to_plot_all_idx]
    phase_no_otu1 = numpy.asarray(param_no_otu1_dict['phase_leastsq'][data_type])[to_plot_no_otu1_idx]
    ...
    ax_amp = plt.subplot2grid((4, len(data_type_all)), (0, data_type_idx))
    ax_freq = plt.subplot2grid((4, len(data_type_all)), (1, data_type_idx))
    ax_phase = plt.subplot2grid((4, len(data_type_all)), (2, data_type_idx))
    ax_scaled_time = plt.subplot2grid((4, len(data_type_all)), (3, data_type_idx))
    ...
    #if clr_status == True:
    #    ax_amp.set_xscale('log', basex=10)
...
fig_name = "%ssine_parameter_%sno_otu1.png" % (config.analysis_directory, clr_status_label_dict[clr_status])
