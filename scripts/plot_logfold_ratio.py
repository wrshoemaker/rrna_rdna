...
for chunk_idx, chunk in enumerate(chunk_all):
    ...
    for c_idx, c in enumerate(chunk):
        ...
        log_ratio_dna_i = log_ratio_dna[c,:]/delta_t
        log_ratio_rna_i = log_ratio_rna[c,:]/delta_t
        ...
        ax.hist(log_ratio_dna_i, bins=12, density=True, histtype='step', alpha=1, lw=3, color=utils.dna_rna_color_dict['DNA'], zorder=1, label='DNA')
        ax.hist(log_ratio_rna_i, bins=12, density=True, histtype='step', alpha=1, lw=3, color=utils.dna_rna_color_dict['RNA'], zorder=1, label='RNA')

        ax.axvline(x=0, ls=':', c='k', lw=2, zorder=2, label='Stationarity')
        ax.set_yscale('log', basey=10, nonposy='clip')
        ax.set_xlabel("Log-fold change in " + r'$\hat{x}_{i}(t)$' +  '\nb/w timepoints, ' + r'$\Delta \ell$', fontsize=10)
